...
* The model shows better performance after training than the initial pre-trained model - especially in detecting pedestrians (s. fig. 1.2), but the overall PascalBoxes_Precision/mAP@0.5IOU performance seems to be still very low.
...
* A larger batch size reduces the training and evaluation loss and yields a noticable improvement of the PascalBoxes_Precision/mAP@0.5IOU performance.
* However, in general model performance w.r.t. PascalBoxes_Precision/mAP@0.5IOU is still quite low.
...
* Actually, it was expected that color-based augmentation might noticably improve the training results. However, the overall results only improve a tiny bit compared to experiment_0_04. The PascalBoxes_PerformanceByCategory/AP@0.5IOU performance on vehicles even decreases a little, whereas the PascalBoxes_PerformanceByCategory/AP@0.5IOU performance on pedestrians slightly increases.
...
* PascalBoxes_PerformanceByCategory/AP@0.5IOU performance on cyclists is still NaN leading to the conclusion that they are underrepresented in the training data such they are not adequately learned by the model.
...
* Here we get a noticable improvement compared to experiment_0_04, experiment_0_06 or experiment_0_07 - especially w.r.t. PascalBoxes_PerformanceByCategory/AP@0.5IOU on pedestrians. Changing the object size, aspect ratio or location seems to have a stronger effect on the training as color distortions.
...
* Compared to experiment_0_00, the model change (including the adapted learning rate) leads to a noticably higher PascalBoxes_Precision/mAP@0.5IOU performance.
